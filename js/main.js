/**
 * ならやさしいイベントナビ v5.6 - メイン機能（徒歩時間修正版）
 * 更新日: 2025-12-14
 * 修正内容: 徒歩時間を時速4km（ゆっくり歩く速度）で計算
 *   - GeoJSONから全64駅の正確な座標を取得
 *   - 会場の最寄り駅を自動計算
 *   - 駅→会場の徒歩ルートを表示
 *   - すべての駅を地図上に表示
 */
...
async function showRouteInfo(event) {
    ...
    try {
        ...
        if (route) {
            ...
            // 時速4km（ゆっくり歩く速度）で計算: 距離(m) ÷ 分速66.67m/分
            const duration = Math.ceil(route.distance / 66.67);
